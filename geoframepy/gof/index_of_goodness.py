...
This is used to compute the mean of the experimental variogram
...
def compute_index_of_goodness(df_obs, df_sim):

	'''
	compute the index of goodness for simulated timeseries
	
	:param df_obs: input dataframe of observed data
	:type df_obs: pandas.dataframe
	
	:param df_sim: input dataframe of simulated data
	:type df_sim: pandas.dataframe

	:return pandas dataframe containing the index of goodness
	'''
	
	obs_data = np.array(df_obs)
	sim_data = np.array(df_sim)

	index_rmse = (rmse(obs_data, sim_data))[0] #(evaluator(rmse,obs_data, sim_data))[0]

	index_kge = (kge(obs_data, sim_data)[0])[0] #(evaluator(kge,obs_data, sim_data)[0])[0]

	index_nse = (nse(obs_data, sim_data))[0] #(evaluator(nse,obs_data, sim_data))[0]

	index_pbias = (pbias(obs_data, sim_data))[0] #(evaluator(pbias,obs_data, sim_data))[0]

	index_mare = (mare(obs_data, sim_data))[0] #(evaluator(mare,obs_data, sim_data))[0]
	
	index_of_goodness = ['RMSE', 'KGE', 'NSE', 'PBIAS', "MARE"]
	index_values = [index_rmse, index_kge, index_nse, index_pbias, index_mare]

	df = pd.DataFrame([index_values],  columns = index_of_goodness)
...
def kgeprime(simulation_s, evaluation):
    ...
    gamma = (np.std(simulation_s) / sim_mean) / \
        (np.std(evaluation) / obs_mean)
    # calculate error in volume beta (bias of mean discharge)
    beta = np.mean(simulation_s,) / np.mean(evaluation)
